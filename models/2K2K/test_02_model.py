...
def main():
    ...
    with torch.no_grad():
        for f_i_name in tqdm(data_list): 
            ...
            # 读取原始图像
            image = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)  # 使用IMREAD_UNCHANGED以保留alpha通道
            img_rembg = image
            if image.shape[2] == 4:  # 确保图像是RGBA
                # 从RGBA图像创建掩码
                alpha_channel = image[:, :, 3]
                mask = alpha_channel.astype(bool)
                ...
                # 转换人物图像，假设transform_human是已经定义的
                image = transform_human(image)
                ...
                # 处理非RGBA图像的情况，或输出错误消息
                print("Error: Image is not in RGBA format.")
            ...
            image_back = np.full_like(image_back, 128, dtype=np.uint8) #turn gray
